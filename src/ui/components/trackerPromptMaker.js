...
export class TrackerPromptMaker {
	...
	constructor(existingObject = {}, onTrackerPromptSave = () => {}) {
		...
		this.fieldIdentityMode = new Map(); // Tracks whether IDs are autogenerated or explicit.
		this.labelSequence = 1; // Counter for default label seeds.
		this.idSequence = 1; // Counter for fallback ID generation.
	...
	resetIdentityTracking() {
		this.fieldIdentityMode.clear();
		this.labelSequence = 1;
		this.idSequence = 1;
	}

	generateDefaultLabel() {
		const baseLabel = t("trackerPromptMaker.defaults.newFieldLabel", "New Field");
		let suffix = this.labelSequence;
		const existingLabels = new Set(this.collectFieldLabels());
		let candidate = `${baseLabel} ${suffix}`;
		while (existingLabels.has(candidate)) {
			suffix += 1;
			candidate = `${baseLabel} ${suffix}`;
		}
		this.labelSequence = suffix + 1;
		return candidate;
	}

	collectFieldIds(skipFieldKey = null, source = this.backendObject) {
		const ids = new Set();
		const traverse = (obj) => {
			Object.entries(obj || {}).forEach(([backendKey, data]) => {
				if (skipFieldKey && backendKey === skipFieldKey) {
					return;
				}
				if (data?.id) {
					ids.add(data.id);
				}
				if (data?.nestedFields) {
					traverse(data.nestedFields);
				}
			});
		};
		traverse(source);
	...
	collectFieldLabels(skipFieldKey = null, source = this.backendObject) {
	...
	sanitizeLabelToId(label) {
		...
		if (!candidate) {
			candidate = raw.replace(/[^A-Za-z0-9]/g, "");
		}
		if (!candidate) {
			return "";
		}
	...
	generateFallbackId() {
		const nextValue = this.idSequence;
		this.idSequence += 1;
		return `Field${nextValue}`;
	}

	generateUniqueFieldId(label, skipFieldKey = null) {
		const existingIds = this.collectFieldIds(skipFieldKey);
		let baseId = this.sanitizeLabelToId(label);
		if (!baseId) {
			baseId = this.generateFallbackId();
		}
	...
	deriveLabelFromId(id) {
		if (!id) {
			return this.generateDefaultLabel();
		}
		const spaced = id
	...
	updateFieldIdBadge(fieldWrapper, newId) {
		if (!fieldWrapper || fieldWrapper.length === 0) {
			return;
		}
		const badge = fieldWrapper.find(".field-id-badge");
		if (badge.length) {
			badge.text(newId || "");
		}
	}
	...
	addField(parentObject = null, parentFieldId = null, fieldData = {}, fieldId = null, isNewField = true) {
		...
		if (!fieldData.label && typeof fieldData.name === "string") {
			fieldData.label = fieldData.name;
		}

		const explicitIdProvided = typeof fieldData.id === "string" && fieldData.id.trim() !== "";
		let sanitizedId = explicitIdProvided ? fieldData.id.trim() : "";
		if (!sanitizedId && typeof fieldData.name === "string") {
			sanitizedId = this.sanitizeLabelToId(fieldData.name);
		}
		let labelValue = typeof fieldData.label === "string" ? fieldData.label : "";
		if (!labelValue) {
			labelValue = sanitizedId ? this.deriveLabelFromId(sanitizedId) : this.generateDefaultLabel();
		}
		let identityMode = explicitIdProvided && !isNewField ? "explicit" : "generated";
		if (!sanitizedId) {
			sanitizedId = this.generateUniqueFieldId(labelValue, fieldId);
		} else if (identityMode === "generated") {
			sanitizedId = this.generateUniqueFieldId(labelValue, fieldId);
		}
		fieldData.id = sanitizedId;
		fieldData.label = labelValue;
		delete fieldData.name;
		this.fieldIdentityMode.set(fieldId, identityMode);
		fieldData.nestedFields = fieldData.nestedFields || {};

		const labels = {
			fieldId: t("trackerPromptMaker.labels.fieldName", "Field ID:"),
			fieldLabel: t("trackerPromptMaker.labels.fieldLabel", "Display Label:"),
			fieldLabelPlaceholder: t("trackerPromptMaker.placeholders.fieldLabel", "Display Label"),
		...
		// Field Label Input with ID badge
		const fieldLabelLabel = $("<label></label>").text(labels.fieldLabel);
		const fieldLabelInput = $('<input type="text" class="text_pole field-label-input">')
			.attr("placeholder", labels.fieldLabelPlaceholder)
			.val(fieldData.label || "")
			.on("input", (e) => {
				const currentFieldId = fieldWrapper.attr("data-field-id");
				this.updateFieldLabel(e.target.value, currentFieldId, fieldWrapper);
				this.syncBackendObject();
			});
		const fieldLabelDiv = $('<div class="field-label-wrapper"></div>').append(fieldLabelLabel, fieldLabelInput);
		const fieldIdLabel = $("<span></span>").addClass("field-id-label").text(labels.fieldId);
		const fieldIdBadge = $('<span class="field-badge field-badge--identifier field-id-badge"></span>').text(fieldData.id || "");
		const fieldIdDiv = $('<div class="field-id-wrapper"></div>').append(fieldIdLabel, fieldIdBadge);
		const fieldIdentityContainer = $('<div class="field-identity-container"></div>').append(fieldLabelDiv, fieldIdDiv);
		...
		// Append field identity, presence, and type selectors
		nameDynamicTypeDiv.append(fieldIdentityContainer, presenceDiv, fieldTypeDiv);
		...
		fieldWrapper.data("addNestedFieldBtn", addNestedFieldBtn);
	...
	removeField(fieldId, fieldWrapper) {
		...
		if (fieldData?.metadata?.internal) {
			const fieldLabel = fieldData.label || fieldData.id || fieldId;
			warn(`Attempted to remove internal field "${fieldLabel}" ignored.`);
		...
		if (confirm(t("trackerPromptMaker.messages.confirmRemoveField", "Are you sure you want to remove this field?"))) {
			...
			fieldWrapper.find(".field-wrapper").each((_, element) => {
				const nestedId = $(element).attr("data-field-id");
				if (nestedId) {
					idsToRemove.add(nestedId);
				}
			});
			idsToRemove.forEach((idToDelete) => {
				this.fieldIdentityMode.delete(idToDelete);
			});
	 ...
	 */
	updateFieldLabel(label, fieldId, fieldWrapper = null) {
		const safeLabel = typeof label === "string" ? label : "";
		if (safeLabel.includes('"')) {
			warn("Field label cannot contain double quotes.");
			if (typeof toastr !== "undefined") {
				toastr.error("Field label cannot contain double quotes.");
			}
			return false;
		}
		const fieldData = this.getFieldDataById(fieldId);
		if (fieldData) {
			...
			fieldData.label = trimmed || this.generateDefaultLabel();
			const identityMode = this.fieldIdentityMode.get(fieldId);
			// IDs regenerate when labels change unless the field originated from an imported schema.
			if (identityMode !== "explicit") {
				fieldData.id = this.generateUniqueFieldId(fieldData.label, fieldId);
			}
			if (!fieldWrapper || fieldWrapper.length === 0) {
				fieldWrapper = this.element.find(`[data-field-id="${fieldId}"]`);
			}
			this.updateFieldIdBadge(fieldWrapper, fieldData.id);
			const labelInput = fieldWrapper.find(".field-label-input");
			if (labelInput.length && labelInput.val() !== fieldData.label) {
				labelInput.val(fieldData.label);
			}
			debug(`Updated field label: ${fieldData.label} for field ID: ${fieldId}`);
			return true;
		} else {
	...
	selectFieldType(type, fieldId) {
		...
		if (fieldData) {
			...
			const addNestedFieldBtn = fieldWrapper.data("addNestedFieldBtn") || fieldWrapper.find(".menu_button:contains('Add Nested Field')");
			const isNestingType = TrackerPromptMaker.NESTING_FIELD_TYPES.includes(type);
			if (addNestedFieldBtn && typeof addNestedFieldBtn.toggle === "function") {
	...
	syncBackendOrderWithDOM() {
		...
		const synchronize = (container, backendRef = {}) => {
			...
			container.children(".field-wrapper").each((_, element) => {
				...
				this.updateFieldIdBadge(fieldWrapper, fieldData.id);
				const labelInput = fieldWrapper.find(".field-label-input");
				if (labelInput.length && typeof fieldData.label === "string") {
					labelInput.val(fieldData.label);
				}
