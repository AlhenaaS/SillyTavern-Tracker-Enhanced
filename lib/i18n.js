...
const LOCALE_MANIFEST_FILE = "catalog.json";
...
const manifestAliasRedirects = new Map();
...
function registerManifestAlias(alias, target) {
	const normalizedAlias = normalizeLocaleId(alias);
	const normalizedTarget = normalizeLocaleId(target);
	if (!normalizedAlias || !normalizedTarget) {
		return;
	}
	if (!manifestAliasRedirects.has(normalizedAlias)) {
		manifestAliasRedirects.set(normalizedAlias, normalizedTarget);
	}
}

function applyLocaleManifest(manifest = null) {
	manifestLocales.clear();
	manifestAliasRedirects.clear();
	manifestFileMap.clear();

	const manifestEntries = Array.isArray(manifest?.locales) ? manifest.locales : [];
	for (const entry of manifestEntries) {
		const canonicalId = normalizeLocaleId(entry?.id);
		if (!canonicalId) {
			continue;
		}
		const label = resolveLocaleLabel(entry, canonicalId);
		const fileName = normalizeLocaleId(entry?.file ?? canonicalId) || canonicalId;
		manifestLocales.set(canonicalId, { id: canonicalId, label, file: fileName });
		manifestFileMap.set(canonicalId, fileName);
		localeFilePathCache.set(canonicalId, fileName);

		const aliases = entry?.aliases;
		if (Array.isArray(aliases)) {
			for (const alias of aliases) {
				registerManifestAlias(alias, canonicalId);
			}
		} else if (aliases && typeof aliases === "object") {
			for (const [aliasKey, aliasValue] of Object.entries(aliases)) {
				registerManifestAlias(aliasKey, aliasValue || canonicalId);
			}
		}
		registerManifestAlias(canonicalId, canonicalId);
	}

	if (!manifestLocales.size) {
		for (const fallback of STATIC_FALLBACK_LOCALES) {
			const canonicalId = normalizeLocaleId(fallback?.id);
			if (!canonicalId || manifestLocales.has(canonicalId)) {
				continue;
			}
			const label = resolveLocaleLabel(fallback, canonicalId);
			manifestLocales.set(canonicalId, { id: canonicalId, label, file: canonicalId });
			manifestFileMap.set(canonicalId, canonicalId);
			registerManifestAlias(canonicalId, canonicalId);
		}
	}

	if (!manifestLocales.has(DEFAULT_LOCALE)) {
		manifestLocales.set(DEFAULT_LOCALE, { id: DEFAULT_LOCALE, label: "English", file: DEFAULT_LOCALE });
		manifestFileMap.set(DEFAULT_LOCALE, DEFAULT_LOCALE);
		registerManifestAlias(DEFAULT_LOCALE, DEFAULT_LOCALE);
	}

	for (const [alias, target] of Object.entries(TRACKER_FALLBACK_ALIASES)) {
		registerManifestAlias(alias, target);
	}
}

function getLocaleManifestUrl() {
	return `/${extensionFolderPath}/locales/${LOCALE_MANIFEST_FILE}`;
}

async function ensureLocaleManifest() {
	if (manifestReady) {
		return;
	}
	if (!manifestLoadPromise) {
		manifestLoadPromise = (async () => {
			try {
				const response = await fetch(getLocaleManifestUrl());
				if (!response?.ok) {
					throw new Error(`Manifest fetch failed with status ${response?.status}`);
				}
				const manifestData = await response.json();
				applyLocaleManifest(manifestData);
			} catch (err) {
				warn("Locale manifest load failed; using static fallback list", err);
				applyLocaleManifest({ locales: STATIC_FALLBACK_LOCALES });
			} finally {
				manifestReady = true;
			}
		})();
...
function resolveManifestLocaleId(locale) {
	const normalized = normalizeLocaleId(locale);
	if (!normalized) {
		return null;
	}
	if (manifestLocales.has(normalized)) {
		return normalized;
	}
	if (manifestAliasRedirects.has(normalized)) {
		return manifestAliasRedirects.get(normalized);
	}
	return null;
}

function getManifestLocaleEntry(locale) {
	const canonical = resolveManifestLocaleId(locale);
	if (!canonical) {
		return null;
	}
	return manifestLocales.get(canonical) || null;
}
...
function applyLocaleCatalog(entries = [], aliasEntries = null) {
	...
	for (const entry of entries) {
		...
		const manifestInfo = getManifestLocaleEntry(canonicalId);
		const resolvedId = manifestInfo?.id ?? canonicalId;
		const label = manifestInfo?.label ?? resolveLocaleLabel(entry, resolvedId);
		addLocale(resolvedId, label);
	}
	...
	const mergedAliases = mergeAliasSources(aliasEntries, manifestAliasRedirects, TRACKER_FALLBACK_ALIASES);
...
async function ensureLocaleBundle(locale) {
	...
	await ensureLocaleManifest();
	const canonical = resolveManifestLocaleId(normalized) || (normalized === DEFAULT_LOCALE ? DEFAULT_LOCALE : null);
	...
	if (canonical !== normalized) {
		const availability = await ensureLocaleBundle(canonical);
		localeAssetAvailability.set(normalized, availability);
		if (availability) {
			const canonicalFile = localeFilePathCache.get(canonical);
			if (canonicalFile) {
				localeFilePathCache.set(normalized, canonicalFile);
			}
		}
		return availability;
	}
	...
	if (canonical === DEFAULT_LOCALE) {
		localeAssetAvailability.set(canonical, true);
		return true;
	}
	...
	const candidates = [];
	const manifestCandidate = manifestFileMap.get(canonical);
	if (manifestCandidate) {
		candidates.push(manifestCandidate);
	}
	for (const candidate of getLocaleFileCandidates(canonical)) {
	...
	for (const candidate of candidates) {
		try {
			...
			try {
				...
				debug("Locale bundle JSON parse failed; using empty object", { locale: canonical, error: err?.message });
			}
			localeFilePathCache.set(canonical, candidate);
			if (data && typeof data === "object" && Object.keys(data).length > 0) {
				localeCache.set(canonical, data);
			} else {
				localeCache.set(canonical, {});
			}
			localeAssetAvailability.set(canonical, true);
			return true;
		} catch (err) {
			debug("Locale bundle fetch failed", { locale: canonical, candidate, error: err?.message });
		}
	}

	localeAssetAvailability.set(canonical, false);
	return false;
}
...
async function filterLocalesByAssets(entries = []) {
	...
	for (const entry of entries) {
		const candidateId = normalizeLocaleId(entry?.id ?? entry?.lang ?? entry?.locale ?? entry);
		if (!candidateId) {
			continue;
		}
		const manifestEntry = getManifestLocaleEntry(candidateId);
		if (!manifestEntry) {
			continue;
		}
		if (!filtered.has(manifestEntry.id)) {
			const resolvedLabel = manifestEntry.label ?? resolveLocaleLabel(entry, manifestEntry.id);
			filtered.set(manifestEntry.id, { id: manifestEntry.id, label: resolvedLabel });
		}
	}

	const defaultManifest = getManifestLocaleEntry(DEFAULT_LOCALE);
	if (defaultManifest && !filtered.has(defaultManifest.id)) {
		filtered.set(defaultManifest.id, { id: defaultManifest.id, label: defaultManifest.label });
	}

	if (!filtered.size) {
		for (const fallback of STATIC_FALLBACK_LOCALES) {
			const manifestEntry = getManifestLocaleEntry(fallback.id);
			if (!manifestEntry) {
				continue;
			}
			if (!filtered.has(manifestEntry.id)) {
				filtered.set(manifestEntry.id, { id: manifestEntry.id, label: manifestEntry.label ?? fallback.label });
			}
		}
	}
...
async function loadLocaleFile(locale) {
	...
	if (!canonical) {
		localeAssetAvailability.set(normalized, false);
		const fallbackValue = {};
		localeCache.set(normalized, fallbackValue);
		return fallbackValue;
	}

	if (canonical !== normalized) {
		const data = await loadLocaleFile(canonical);
		localeCache.set(normalized, data);
		const canonicalAvailability = localeAssetAvailability.get(canonical);
		if (canonicalAvailability !== undefined) {
			localeAssetAvailability.set(normalized, canonicalAvailability);
		}
		const canonicalFile = localeFilePathCache.get(canonical);
		if (canonicalFile) {
			localeFilePathCache.set(normalized, canonicalFile);
	...
	if (localeCache.has(canonical)) {
		const cachedValue = localeCache.get(canonical);
		if (!localeAssetAvailability.has(canonical)) {
			const isNonEmptyObject = cachedValue && typeof cachedValue === "object" && Object.keys(cachedValue).length > 0;
			localeAssetAvailability.set(canonical, isNonEmptyObject || canonical === DEFAULT_LOCALE);
		}
		return cachedValue;
	}
	...
	for (const candidate of candidates) {
		try {
			...
			try {
				...
				debug("Locale bundle JSON parse failed", { locale: canonical, candidate, error: err?.message });
			}
			localeCache.set(canonical, data);
			localeFilePathCache.set(canonical, candidate);
			localeAssetAvailability.set(canonical, true);
	...
	warn("Locale file not found; using fallback data", { locale: canonical, attempts });
	const fallbackValue = {};
	localeCache.set(canonical, fallbackValue);
	localeAssetAvailability.set(canonical, canonical === DEFAULT_LOCALE);
	return fallbackValue;
}
