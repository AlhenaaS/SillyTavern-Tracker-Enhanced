...
import { automationTargets } from "../src/settings/settings.js";
import { trackerExists } from "../src/trackerDataHandler.js";

function getSafeExtensionSettings() {
	try {
...
export function debug(...msg) {
	const settings = getSafeExtensionSettings();
...
export function shouldGenerateTracker(mesId, type) {
	...
	const target = extensionSettings.automationTarget ?? automationTargets.BOTH;

	const isUser = message?.is_user ?? false;

	if (target === automationTargets.NONE || mesId < extensionSettings.generateFromMessage) return false;

	let requiredTarget;

	if (type === "impersonate") {
		requiredTarget = [automationTargets.USER, automationTargets.BOTH];
	} else if (type === "continue" || type === "regenerate") {
		if (isUser) {
			requiredTarget = [automationTargets.USER, automationTargets.BOTH];
		} else {
			requiredTarget = [automationTargets.CHARACTER, automationTargets.BOTH];
		}
	} else if (type === "swipe") {
		if (isUser) return false;
		requiredTarget = [automationTargets.CHARACTER, automationTargets.BOTH];
	} else {
		if (isUser) requiredTarget = [automationTargets.USER, automationTargets.BOTH];
		else requiredTarget = [automationTargets.CHARACTER, automationTargets.BOTH];
	}
...
export function shouldShowPopup(mesId, type) {
	...
	const message = chat[mesId];
	const target = extensionSettings.showPopupFor ?? automationTargets.NONE;

	const isUser = message?.is_user ?? false;

	if (target === automationTargets.NONE) return false;
